...
# ])
model = tf.keras.Sequential([
    tf.keras.layers.Input(shape=(4,), name='input'),         # Input: 4 維
    tf.keras.layers.Dense(8, activation='linear', name='dense1'),  # 4 → 8
    tf.keras.layers.Activation('relu', name='relu1'),
    tf.keras.layers.Dense(16, activation='linear', name='dense2'), # 8 → 16
    tf.keras.layers.Activation('relu', name='relu2'),
    tf.keras.layers.Dense(10, activation='linear', name='dense3'), # 16 → 10
    tf.keras.layers.Activation('softmax', name='softmax')
])
