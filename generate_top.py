...
with open(output_path, 'w') as f_out:
    ...
    for i, layer in enumerate(layer_list):
        ...
        if(i==0): cur = 'input'
        else: cur = 'buffers[toggle ^ 1]'
        next = 'buffers[toggle]'

        if ltype == 'Dense':
            f_out.write(f'    dense<{in_dim}, {out_dim}>({cur}, {lname}_kernel, {lname}_bias, {next});\n')
            f_out.write(f'    std::cout << "HLS output after layer {i} ({lname})\\n";\n')
            f_out.write(f'    for (int i = 0; i < {out_dim}; i++) std::cout << {next}[i].to_double() << std::endl;\n')
        elif ltype == 'Activation':
            ...
            if 'relu' in lname_lower:
                ...
                f_out.write(f'    std::cout << "softmax start" << std::endl;\n')
                f_out.write(f'    softmax<{in_dim}>({cur}, {next});\n')
                f_out.write(f'    std::cout << "softmax end" << std::endl;\n')
            else:
